// forked from design-sync lib/dts.mjs — @app-factory/ui re-exports @mantine/core, so the style-system package filter must exempt Mantine's per-component API declarations (lib/components/**) or every real prop is dropped
...
function isOwnProp(p, pkgDir, styleSystemDirs) {
  ...
  if (styleSystemDirs.some((k) => k.endsWith('/') && fp.startsWith(k))) {
    // fork: @mantine/core is flagged as a style-system package (correct for
    // its Box/styles-api layers), but its lib/components/** declarations ARE
    // the DS's real per-component API — keep those, minus DOM-noise names.
    if (!fp.includes('/@mantine/core/lib/components/')) return false;
