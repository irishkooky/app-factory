...
- **`implementer` サブエージェント（Sonnet固定）= 実装担当**
  - テンプレートのコピーなどのスキャフォールド作業
  - 仕様書に基づくコーディング、レビュー指摘の修正
  - その他、高度な判断を要しない細かい作業
  - 起動方法: Agent ツールで `subagent_type: implementer` を指定する
- **`researcher` サブエージェント（Haiku固定）= 調査担当**
  - コードベースの探索、既存実装・ファイル構成の確認
  - ライブラリやAPIのドキュメント調査
  - 起動方法: Agent ツールで `subagent_type: researcher` を指定する

これらのサブエージェントは `.claude/agents/`（実体は `.agents/agents/`）で定義されており、
使用モデルはフロントマターの `model` フィールドで固定されている。そのため呼び出し時に
`model` パラメータを指定する必要はなく、指定し忘れてもメインモデル（Fable/Opus）に
フォールバックすることはない。

運用ルール:
- 実装・調査を委任する時は必ず上記の `subagent_type` を指定する。汎用エージェント
...
- `implementer` の成果物はレビューなしでコミット・デプロイしない
